defmodule Mix.Tasks.SutoMixTasks.Gen.Live do
  @shortdoc "Generates LiveView, templates with Petal Components, and context for a resource"

  @moduledoc """
  Generates LiveView, templates, and context for a resource.

      mix suto.gen.live Accounts User users name:string age:integer
  ...
  Alternatively, the `--context-app` option may be supplied to the generator:
      ...
      mix suto.gen.live Accounts User users --context-app warehouse
  ...
  module name, for example:
      ...
      mix suto.gen.live Accounts User users --web Sales
  ...
  def run(args) do
    if Mix.Project.umbrella?() do
      Mix.raise(
        "mix suto.gen.live must be invoked from within your *_web application root directory"
      )
    end
    ...
    paths = [:suto_mix_tasks | Mix.Phoenix.generator_paths()]
  ...
  def inputs(%Schema{} = schema) do
    ...
    |> Enum.map(fn
      ...
      {key, :integer} ->
        ~s(<.field field={@form[#{inspect(key)}]} type="number" label="#{label(key)}" />)

      {key, :float} ->
        ~s(<.field field={@form[#{inspect(key)}]} type="number" label="#{label(key)}" step="any" />)

      {key, :decimal} ->
        ~s(<.field field={@form[#{inspect(key)}]} type="number" label="#{label(key)}" step="any" />)

      {key, :boolean} ->
        ~s(<.field field={@form[#{inspect(key)}]} type="checkbox" label="#{label(key)}" />)

      {key, :text} ->
        ~s(<.field field={@form[#{inspect(key)}]} type="text" label="#{label(key)}" />)

      {key, :date} ->
        ~s(<.field field={@form[#{inspect(key)}]} type="date" label="#{label(key)}" />)

      {key, :time} ->
        ~s(<.field field={@form[#{inspect(key)}]} type="time" label="#{label(key)}" />)

      {key, :utc_datetime} ->
        ~s(<.field field={@form[#{inspect(key)}]} type="datetime-local" label="#{label(key)}" />)
